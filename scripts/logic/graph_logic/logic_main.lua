...
wario_land_3_location = {}
wario_land_3_location.__index = wario_land_3_location
...
function wario_land_3_location.new(name)
    local self = setmetatable({}, wario_land_3_location)
    if name then
...
function wario_land_3_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = wario_land_3_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function wario_land_3_location:connect_two_ways(exit, rule)
...
function wario_land_3_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function wario_land_3_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function wario_land_3_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function wario_land_3_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function wario_land_3_location:accessibility()
...
-- 
function wario_land_3_location:discover(accessibility, keys)
...
entry_point = wario_land_3_location.new("entry_point")
